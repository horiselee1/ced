...
def _step_amp(
    ...
    def _update(engine, batch):
        ...
        with torch.enable_grad():
            # 修改后：只解包标准波形、标签和文件名，将 specaug 变量设为 None
            x, y, fnames = transfer_to_device(batch)
            specaug_value, specaug_min_value = None, None
...
class Runner(object):
    ...
    def train(self, config: Union[Path, str], **overwrite_kwargs):
        ...
        # 【修改为】：
        spectransforms = torch.nn.Sequential()
        wavtransforms = torch.nn.Sequential()
        ...
        train_dataset = dataset.WeakHDF5Dataset(
            train_df,
            num_classes=config_parameters.num_classes
        )
