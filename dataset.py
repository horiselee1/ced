...
class WeakHDF5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,
        ...
        # 【核心修改】：由于 HDF5 内部只存了纯文件名，我们用 .split('/')[-1] 剥离所有路径前缀
        # 无论是 data/Bird/xxx.wav 还是 data\Construction\xxx.wav，都统一转化为 xxx.wav
        hdf5_fname = str(fname).replace('\\', '/').split('/')[-1]
        
        try:
            # 尝试使用纯文件名读取
            data = self._readdata(hdf5path, hdf5_fname)
        except KeyError:
            # 如果万一还是找不到，退回到模糊匹配机制
            if hdf5path not in self._datasetcache:
                from h5py import File
                self._datasetcache[hdf5path] = File(hdf5path, 'r')
            
            db = self._datasetcache[hdf5path]
            db_keys = list(db.keys())
            
            clean_target = hdf5_fname.lower().strip('/')
            matched_key = None
            
            for key in db_keys:
                if key.lower().strip('/') == clean_target:
                    matched_key = key
                    break
            
            if matched_key is not None:
                data = self._readdata(hdf5path, matched_key)
            else:
                # 彻底失败时打印提示
                print(f"\n[数据匹配失败严重警告]!")
                print(f"提取出的纯文件名: '{hdf5_fname}' (原始路径: '{fname}')")
                print(f"该HDF5文件 ({hdf5path}) 内部实际包含的前3个Key为: {db_keys[:3]}")
                raise KeyError(f"在 HDF5 中找不到对应的音频数据: {hdf5_fname}")
